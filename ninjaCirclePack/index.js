const data = [
  ...
  // define stratify with id & parentId
  const stratify=d3.stratify()
  .id((d)=>d.name)
  .parentId(d=>d.parent)
  
  const rootNode = stratify(data)
  .sum(d => d.amount);// what value summed for parent nodes , determines size 

  const pack = d3.pack()
  .size([960, 700])
  .padding(5);

  // convert packed data back to array of docs
  const bubbledData=pack(rootNode).descendants();
...
// attach circle + text to each Group
nodes.append('circle')
  .attr('r', d => d.r)
  .attr('stroke', 'white')
  .attr('stroke-width', 2)
  .attr('fill', d => colour(d.depth)); // fill from scale based on depth

// filter nodes wihtout children
// append text on node
nodes=nodes.filter(node=>{return !node.children})
.append('text')
.attr('text-anchor', 'middle')
.attr('dy','0.3em')
.attr('fill', 'white')
.text(d=>d.data.name)
.style('font-size',d=>d.value*5)// d.value is summation amount var 
console.log(nodes)
